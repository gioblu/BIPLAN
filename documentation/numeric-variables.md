...
## Numeric variables
BIPLAN supports only one signed numeric variable type, that is by default `long` (if a different type is required see [configuration](/documentation/configuration.md)). Numeric variables are identified by `@`. The name of variables must be composed by lowercase and or uppercase letters, numbers and or the symbol `_`. Each variable is just an entry of a global array of variables. BIPLAN supports up to 87 variables.
