...
int main(int argc, char* argv[]) {
  ...
  if(p_file == NULL) {
    printf("| State    |\033[31m ⚠️ Error: Unable to open source file.\033[m\n\n");
    exit(2);
  }
  // Obtain file size:
  ...
  if(result != p_size) {
    printf("| State    |\033[31m ⚠️ Error: Unable to read source file.\033[m\n\n");
    exit(4);
  }
  ...
  if(strlen(program) > 0) {
    ...
    if(o_file == NULL) {
      printf("| State    |\033[31m ⚠️ Error: Unable to open the target file.\033[m\n\n");
      exit(5);
    }
