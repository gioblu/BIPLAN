...
if ! command -v php >/dev/null 2>&1; then
    echo "php not found, the benchmark cannot be run without it, please install it!"
    exit 1
fi
...
do
    ...
    for ((it=0; it<${iterations}; it++ ));
    do
        php "$SCRIPT_DIR/programs/${tests[$i - 1]}.py"  > /dev/null 2>&1
    done
    end_py=$(date +%s%6N)
...
if [ $total_bip -lt $total_py ]; then
    ...
    echo -e "\nbiplan is $ratio times faster than php.\n"
else
    ratio=$(echo "scale=2; $total_bip/$total_py" | bc)
    echo -e "\nphp is $ratio times faster than biplan.\n"
fi